    ...
    while hq:
        ...
        for i in range(4):
            ...
            if 0 <= nx < N and 0 <= ny < N:
                now_cost = cost + ((room_map[nx][ny]-1)*-1)
                if now_cost < cost_list[nx][ny]:
                    cost_list[nx][ny] = now_cost
                    heapq.heappush(hq,(now_cost,nx,ny))

    print(cost_list[N-1][N-1])
    return
...
room_map = []
for _ in range(N):
    room_map.append(list(map(int,stdin.readline().strip())))
